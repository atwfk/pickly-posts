...
const  Header=(props) =>{
    const {userName="Ahmed Ayoub",
        timeOfPost="2 hours ago",
        srcOfUserImage=avatar
        
    }=props;
    return (
        <div class="flex justify-between pb-s border-b w-full border-grey-shd6 mb-m">
          <div class="flex">
            <img
              src={srcOfUserImage}
              alt="filled avatar"
              class="w-10 h-10 mr-m"
            />
            <div>
              <h4 class="font-medium text-sm text-dark">{userName}</h4>
              <span class="font-light text-sm text-dark-grey">
                {timeOfPost}
              </span>
            </div>
          </div>
